...
public class ChatTests : IntegrationTestBase
{
    ...
    [Test]
    public async Task ShadowBannedUser_CanLogin()
    {
    ...
    [Test]
    public async Task ShadowBannedUser_MessageNotBroadcast()
    {
        ...
        // Verify message is not in history and sent to caller
        var messages = _chatHistory.GetMessages("W3C Lounge");
        Assert.AreEqual(0, messages.Count);
        Assert.IsNotNull(_capturedCallerMessage);
        Assert.AreEqual("Hello world", _capturedCallerMessage.Message);

        // Verify message is NOT broadcast to group
        Assert.IsFalse(_groupMessageSent, "Shadow banned message should not be broadcast to group");
    }
    ...
    public async Task DeleteMessage_ValidMessage_DeletesAndNotifiesClients()
    {
        ...
        _clients.Setup(c => c.All).Returns(mockAllProxy.Object);
        ...
        mockAllProxy.Verify(p => p.SendCoreAsync("MessageDeleted",
            It.Is<object[]>(args => args.Length == 1 && args[0].Equals(message.Id)),
            default), Times.Once);
    }

    [Test]
    public async Task DeleteMessage_NonExistentMessage_DoesNothing()
    {
        // Arrange
        await LoginUser();
        var mockAllProxy = new Mock<IClientProxy>();
        ...
        // Assert
        mockAllProxy.Verify(p => p.SendCoreAsync(It.IsAny<string>(), It.IsAny<object[]>(), default),
            Times.Never);
    }
    ...
    public async Task PurgeMessagesFromUser_ExistingUser_DeletesAllMessagesAndNotifies()
    {
        ...
        mockAllProxy.Verify(p => p.SendCoreAsync("BulkMessageDeleted",
            It.Is<object[]>(args => args.Length == 1),
            default), Times.Once);
    }

    [Test]
    public async Task PurgeMessagesFromUser_UserWithNoMessages_DoesNothing()
    {
        // Arrange
        await LoginUser();
        ...
        mockAllProxy.Verify(p => p.SendCoreAsync("BulkMessageDeleted", It.IsAny<object[]>(), default),
            Times.Never);
    }
